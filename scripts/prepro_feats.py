...
def train(i,indices,imgs,params,dir_fc,dir_att):
    if i % 4 == 0:
    ...
    my_resnet.cuda()
    my_resnet.eval()
    for j in tqdm(range(indices[i+1]-indices[i])):
        # load the image
        img = imgs[j+indices[i]]
        if os.path.isfile('data/all_att/'+str(j+indices[i])+'.npz'):
            continue
...
#             tmp_fc, tmp_att = my_resnet(I, params['att_size'])
        ...
        # write to pkl
#         np.save(os.path.join(dir_fc,str(j+indices[i]) ), tmp_fc.data.cpu().float().numpy())
        np.savez_compressed(os.path.join(dir_att, str(j+indices[i])), feat=tmp_att.data.cpu().float().numpy())
...
if __name__ == "__main__":
    ...
    indices = [int(np.ceil(i* length)) for i in range(n_processes+1)]
    set_start_method('spawn')
    for i in range(n_processes):
        p = 'process'+str(i)
        p = mp.Process(target=train,args=(i,indices,imgs,params,dir_fc,dir_att,))
        p.start()
